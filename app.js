...
// ======= Color HSL que sigue al cursor (DEMO) =======
// Chicos: mueve el mouse lateralmente y cambia el tono (H) en tiempo real.
(function(){
  const root = document.documentElement; // <html>
  let enabled = true; // Puedes apagarlo con la tecla "t" si distrae

  // (Opcional) Tecla "t" para activar/desactivar en vivo
  ...
  // Transición más fluida (suaviza el cambio de color)
  root.style.transition = 'background-color 0.3s ease, color 0.3s ease, filter 0.3s ease';

  // Movimiento del mouse -> cambia el tono HSL
  window.addEventListener('mousemove', (e) => {
    if (!enabled) return;
    const w = window.innerWidth || 1;
    const hue = Math.round((e.clientX / w) * 360); // 0..360 (mapea la posición X del cursor)
